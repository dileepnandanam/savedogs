...
module Api
  class DogUpdatesController < Api::BaseController
    def index
    ...
    def create
      ...
      if @dog_update.save
        ...
        render json: @dog_update.errors.messages, status: :unprocessable_entity
      end
    end

    def destroy
      @dog_update = DogUpdate.where(id: params[:id], user_id: current_user.id).first
      if @dog_update&.update(state: 'deleted')
    ...
    def image_url(dog_update)
      return '' if dog_update.image.blank?
